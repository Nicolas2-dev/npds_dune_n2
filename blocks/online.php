...
function online()
{
    ...
    if ($row = sql_fetch_row($result)) {
        sql_query("UPDATE " . sql_prefix('session') . " SET username='$username', time='$ctime', host_addr='$ip', guest='$guest' WHERE username='$username'");
    } else {
        sql_query("INSERT INTO " . sql_prefix('session') . " (username, time, host_addr, guest) VALUES ('$username', '$ctime', '$ip', '$guest')");
    }

    $result = sql_query("SELECT username FROM " . sql_prefix('session') . " WHERE guest=1");
    $guest_online_num = sql_num_rows($result);

    $result = sql_query("SELECT username FROM " . sql_prefix('session') . " WHERE guest=0");
    ...
    if ($user) {
        ...
        $result = Q_select("SELECT uid FROM " . sql_prefix('users') . " WHERE uname='$username'", 86400);
        $uid = $result[0];

        $result2 = sql_query("SELECT to_userid FROM " . sql_prefix('priv_msgs') . " WHERE to_userid='" . $uid['uid'] . "' AND type_msg='0'");
