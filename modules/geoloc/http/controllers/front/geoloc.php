...
if (Auth::autorisation(-127)) {
    ...
    if ($geo_ip == 1) {
        ...
        $result_postip = sql_query('SELECT COUNT(*) AS nbpost , poster_ip 
        ...
        $result_ip = sql_query('SELECT * 
                                FROM ' . sql_prefix('ip_loc') . ' 
                                ORDER BY ip_visite DESC');
        ...
        $sidebarip = '<div id="sb_ip" class="list-group mb-2">
        ...
        $f = fopen('modules/' . $ModPath . '/views/iplist.html', 'w');

        $w = fwrite($f, $tab_ip);

        @fclose($f);

        @chmod('modules/' . $ModPath . '/views/iplist.html', 0666);
    }
}
// for admin
...
if (isset($cookie)) {
    ...
    if ($found == 0) {
        $res = sql_query("INSERT INTO " . sql_prefix('users_extend') . " 
                          VALUES ('$uid','','','','','','','','','','','','','')");
    }
}
...
if (array_key_exists('mod', $_GET)) {
    if ($f_new_lat != '' and $f_new_long != '' and $f_geomod = "neo") {
        sql_query('UPDATE ' . sql_prefix('users_extend') . ' 
                   SET ' . $ch_lat . ' = "' . $f_new_lat . '", ' . $ch_lon . ' = "' . $f_new_long . '" 
                   WHERE uid = "' . $uid . '"');
    }
...
$result_con = sql_query('SELECT * 
                         FROM ' . sql_prefix('session') . ' s');
...
while ($row = sql_fetch_array($result)) {
    ...
    if (!$short_user) {
        include_once 'modules/reseaux-sociaux/config/config.php';
    ...
    if ($user_lat != '') {
        ...
        /*
        $ano_o_conn.= '[['.$user_long.','.$user_lat.'],"'. addslashes($users_uname) .'","'.$session_host_addr.'","'.$ip_visi_pag.'","'.$ip_visite.'","'.$ip_city1.'","'.$ip_country1.'","'.$ch_img.'flags/'.strtolower($ip_code_country1).'","'.@gethostbyaddr($ip_ip1).'","AM","'.$av_ch.'", "'.addslashes($imm).'","'.addslashes($my_rsos[$krs]).'"],';
        
        $sidebaronline .= '<a id="a'.$i.'" style="min-height:38px;" class="sb_js sb_ano list-group-item list-group-item-action py-1 px-2 border-left-0 border-right-0 small">'.$ic_b_mbgc.'<span class="ms-2"><span class="float-end">'.addslashes($users_uname).'</span></span></a>';
        */
    ...
    $tres = sql_query("SELECT * 
                       FROM " . sql_prefix('ip_loc') . " i 
                       WHERE ip_ip LIKE \"$session_host_addr\"");
    ...
    while ($row1 = sql_fetch_array($tres)) {
        ...
        if ($ip_lat1 != 0 and $ip_long1 != 0) {
            if (!strstr($ip_ip1, $mb_con_g)) {
                ...
                $ano_o_conn .= '[[' . $ip_long1 . ',' . $ip_lat1 . '],"' . $ip_ip1 . '","' . $session_host_addr . '","' . $ip_visi_pag . '","' . $ip_visite . '","' . $ip_city1 . '","' . $ip_country1 . '","' . $ch_img . 'flags/' . strtolower($ip_code_country1) . '","' . @gethostbyaddr($ip_ip1) . '","A"],';

                $sidebaronline .= '<a id="a' . $i . '" style="min-height:38px;" class="sb_js sb_ano list-group-item list-group-item-action py-1 px-2 border-left-0 border-right-0 small">' . $ic_b_acg . '<span class="ms-2">' . $i . '<span class="float-end">' . $ip_ip1 . '</span></span></a>';
            } else {
                $ano_o_conn .= '[[' . $ip_long1 . ',' . $ip_lat1 . '],"' . addslashes($users_uname) . '","' . $session_host_addr . '","' . $ip_visi_pag . '","' . $ip_visite . '","' . $ip_city1 . '","' . $ip_country1 . '","' . $ch_img . 'flags/' . strtolower($ip_code_country1) . '","' . @gethostbyaddr($ip_ip1) . '","AM"],';
...
$sidebaronline = '<div id="sb_ano" class="list-group mb-2">
    <div class="bg-light text-dark fw-light p-2"><a id="carrets_ac" class="link" data-bs-toggle="collapse" href="#l_sb_ano"><i class="toggle-icon fa fa-caret-down fa-lg me-2" style="font-size:1.6rem;"></i></a>
        ...
        <div class="collapse" id="l_sb_ano">
            ' . $test_ip
    . $sidebaronline . '
        </div>
    </div>';
...
while ($row = sql_fetch_array($membre)) {
    ...
    if ($us_lat != '') {
        ...
        $cont_geojson .= '{"type": "Feature", "id": "u_' . $us_uid . '", "properties": { "name": "' . addslashes($us_uname) . '", "description": ""}, "geometry": { "type": "Point", "coordinates": [' . $us_long . ',' . $us_lat . '] } },';
        ...
        $sidebarmembres .= '<a id="u' . $us_uid . '" style="min-height:38px;" class="filtrmb_js sb_js sb_member list-group-item list-group-item-action py-1 px-2 border-left-0 border-right-0 small">' . $ic_b_mbg . '<span class="ms-2 nlfiltrmb" data-searchterm="' . addslashes($us_uname) . '">' . addslashes($us_uname) . '</span><img alt="avatar' . addslashes($us_uname) . '" class="float-end n-ava-32" src="' . $av_ch . '" loading="lazy"/></a>';

        $mbr_geo_off .= '[[' . $us_long . ',' . $us_lat . '], "u' . $us_uid . '","' . addslashes($us_uname) . '","' . $av_ch . '","' . $visit . '","' . addslashes($imm) . '"],';

        $mbr_geo_off_v .= 'var u' . $us_uid . ' = ol.proj.transform([' . $us_long . ', ' . $us_lat . '], "EPSG:4326", "EPSG:3857");';
    }
...
$sidebarmembres = '<div id="sb_member" class="list-group mb-2">
...
@chmod('modules/' . $ModPath . '/storage/data.json', 0666);

// construction du fichier json
$f = fopen('modules/' . $ModPath . '/storage/user.geojson', 'w');
...
foreach ($fond_provider as $k => $v) {
    ...
    switch ($k) {
        ...
        case '4':
            $optcart .= '<optgroup label="Azure Maps">';
            break;
        ...
        case '12':
            $optcart .= '<optgroup label="Stadia Maps">';
            break;
    }
...
if ($op == 'allip' and $geo_ip == 1 and Auth::autorisation(-127)) {
    $ecr_scr .= '
    ip_cluster.setVisible(true);
    grouputilisateurs.getLayers().forEach(l=>{l.setVisible(false)});
...
$affi = '
    ...
    <div class=" mb-4">
        <div id="map-wrapper" class="ol-fullscreen my-3">
            <div id="map" lang="' . Language::languageIso(1, 0, 0) . '" class="map" tabindex="20">
