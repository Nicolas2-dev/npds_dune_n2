...
function autorise_mod($lid, $aff)
{
    ...
    if ($admin) {
        ...
        // faut remettre le controle des droits probablement pour les admin qui ont le droit link ??!!
        if ($radminsuper == 1) {
            if ($aff) {
        ...
        $resultX = sql_query("SELECT submitter 
                              FROM " . $links_DB . "links_links 
                              WHERE submitter='$cookie[1]' 
                              AND lid='$lid'");
...
function index()
{
    ...
    if (($cache_obj->genereting_output == 1) or ($cache_obj->genereting_output == -1) or (!$SuperCache)) {
        ...
        if ($result) {
            while (list($cid, $title, $cdescription) = sql_fetch_row($result)) {
                ...
                $result2 = sql_query("SELECT sid, title 
                                      ...
                                      WHERE cid='$cid' 
                                      ORDER BY title $subcat_limit");

                while (list($sid, $stitle) = sql_fetch_row($result2)) {

                    $cresult3 = sql_query("SELECT lid 
                                           FROM " . $links_DB . "links_links 
                                           WHERE sid='$sid'");
        ...
        if ($result) {
            ...
            echo '<p class="lead" align="center"><span>' . translate('Il y a') . ' <b>' . $numrows . '</b> ' . translate('Liens') . '
    ...
    global $admin;
    if ($admin) {
        $result = sql_query("SELECT requestid 
        ...
        if ($result) {
            ...
            $result2 = sql_query("SELECT requestid 
                                  FROM " . $links_DB . "links_modrequest 
                                  WHERE brokenlink=0");
            ...
            echo '<p class="lead p-2 text-center border rounded bg-light">
...
function viewlink($cid, $min, $orderby, $show)
{
    ...
    if (($cache_obj->genereting_output == 1) or ($cache_obj->genereting_output == -1) or (!$SuperCache)) {
        ...
        $affsouscat .= '<ul class="list-group">
        ...
        while (list($sid, $title) = sql_fetch_row($subresult)) {
            $result2 = sql_query("SELECT lid 
                                  FROM " . $links_DB . "links_links 
                                  WHERE sid='$sid'");

            $numrows_lst = sql_num_rows($result2);

            $affsouscat .= '<li class="list-group-item list-group-item-action justify-content-between align-self-start"><a href="modules.php?ModStart=' . $ModStart . '&amp;ModPath=' . $ModPath . '&amp;op=viewslink&amp;sid=' . $sid . '">' . aff_langue($title) . '</a></li>';
        }
        ...
        $fullcountresult = sql_query("SELECT lid, title, description, date, hits 
                                      FROM " . $links_DB . "links_links 
                                      WHERE cid='$cid' 
                                      AND sid=0");
...
function viewslink($sid, $min, $orderby, $show)
{
    ...
    if (($cache_obj->genereting_output == 1) or ($cache_obj->genereting_output == -1) or (!$SuperCache)) {
        ...
        $filen = 'modules/' . $ModPath . '/support/links.ban_03.php';
...
function categorynewlinkgraphic($cat)
{
    ...
    if ($OnCatNewLink == '1') {
        $newresult = sql_query("SELECT date 
                                FROM " . $links_DB . "links_links 
                                WHERE cid='$cat' 
                                ORDER BY date DESC 
                                LIMIT 1");
...
function detecteditorial($lid, $ttitle)
{
    ...
    $resulted2 = sql_query("SELECT adminid 
...
function viewlinkeditorial($lid, $ttitle)
{
    ...
    $result2 = sql_query("SELECT url 
                          FROM " . $links_DB . "links_links 
                          WHERE lid='$lid'");

    list($url) = sql_fetch_row($result2);

    $result = sql_query("SELECT adminid, editorialtimestamp, editorialtext, editorialtitle 
                         FROM " . $links_DB . "links_editorials 
                         WHERE linkid = '$lid'");
    ...
    if ($recordexist != 0) {
        while (list($adminid, $editorialtimestamp, $editorialtext, $editorialtitle) = sql_fetch_row($result)) {
            ...
            echo '<h4>' . aff_langue($editorialtitle) . '</h4>
...
switch ($op) {
    ...
    case 'AddLink':
        include_once 'modules/' . $ModPath . '/support/links-1.php';
        AddLink();
        break;
    ...
    case 'NewLinks':
        include_once 'modules/' . $ModPath . '/support/links-2.php';
    ...
    case 'brokenlink':
        include_once 'modules/' . $ModPath . '/support/links-3.php';
