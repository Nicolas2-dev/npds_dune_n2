...
function links()
{
    ...
    */
    ...
    $result = sql_query("SELECT * 
                         FROM " . $links_DB . "links_modrequest 
                         WHERE brokenlink=1");
    ...
    $result = sql_query("SELECT lid, cid, sid, title, url, description, name, email, submitter, topicid_card 
...
function LinksAddLink($new, $lid, $title, $url, $cat, $description, $name, $email, $submitter, $topicL)
{
    ...
    if ($title == '') {
        ...
        echo '<div class="alert alert-danger">' . translate('Erreur : vous devez saisir un titre pour votre lien') . '</div>
    ...
    if ($description == '') {
        ...
        echo '<div class="alert alert-danger">' . translate('Erreur : vous devez saisir une description pour votre lien') . '</div>
            <a href="modules.php?ModStart=' . $ModStart . '&amp;ModPath=' . $ModPath . '" class="btn btn-secondary">' . translate('Retour en arrière') . '</a>';
    ...
    echo '<div class="alert alert-success">' . translate('Nouveau lien ajouté dans la base de données') . '</div>
        <a href="modules.php?ModStart=' . $ModStart . '&amp;ModPath=' . $ModPath . '" class="btn btn-secondary">' . translate('Retour en arrière') . '</a>';

    if ($new == 1) {
        sql_query("DELETE FROM " . $links_DB . " links_newlink 
                   WHERE lid='$lid'");
...
function LinksModLink($lid, $modifylinkrequest_adv_infos)
{
    ...
    $result = sql_query("SELECT cid, sid, title, url, description, name, email, hits, topicid_card 
                         FROM " . $links_DB . "links_links 
                         WHERE lid='$lid'");

    echo '<h2>' . translate('Modifier les liens') . '</h2>
    ...
    while (list($cid, $sid, $title, $url, $description, $name, $email, $hits, $topicid_card) = sql_fetch_row($result)) {
        ...
        $resulted2 = sql_query("SELECT adminid, editorialtimestamp, editorialtext, editorialtitle 
                                FROM " . $links_DB . "links_editorials 
                                WHERE linkid='$lid'");
...
function LinksDelLink($lid)
{
    ...
    sql_query("DELETE FROM " . $links_DB . "links_editorials 
               WHERE linkid='$lid'");

    sql_query("DELETE FROM " . $links_DB . "links_links 
               WHERE lid='$lid'");
}
...
function LinksAddEditorial($linkid, $editorialtitle, $editorialtext)
{
    ...
    sql_query("INSERT INTO " . $links_DB . "links_editorials 
...
function LinksAddSubCat($cid, $title)
{
    ...
    if ($numrows > 0) {
        ...
        sql_query("INSERT INTO " . $links_DB . "links_subcategories 
...
function LinksAddCat($title, $cdescription)
{
    ...
    if ($numrows > 0) {
        ...
        sql_query("INSERT INTO " . $links_DB . "links_categories 
...
function LinksModCatS($cid, $sid, $sub, $title, $cdescription)
{
    ...
    if ($sub == 0) {
        sql_query("UPDATE " . $links_DB . "links_categories 
                   SET title='$title', cdescription='$cdescription' WHERE cid='$cid'");
    } else {
        sql_query("UPDATE " . $links_DB . "links_subcategories 
                   SET title='$title' WHERE sid='$sid'");
    }
...
function LinksDelCat($cid, $sid, $sub, $ok = 0)
{
    ...
    if ($ok == 1) {
        ...
        if ($sub > 0) {
            ...
            sql_query("DELETE FROM " . $links_DB . "links_subcategories 
                       WHERE sid='$sid'");
            ...
            sql_query("DELETE FROM " . $links_DB . "links_categories 
                       WHERE cid='$cid'");
...
function LinksListModRequests()
{
    ...
    $resultX = sql_query("SELECT requestid, lid, cid, sid, title, url, description, modifysubmitter, topicid_card 
                          FROM " . $links_DB . "links_modrequest 
                          WHERE brokenlink=0 
                          ORDER BY requestid");
    ...
    echo '<h3 class="mb-3">' . translate('Requête de modification d\'un lien utilisateur') . ' <span class="badge bg-danger float-end">' . $totalmodrequests . '</span></h3>
        <hr class="mb-0" />
    ...
    while (list($requestid, $lid, $cid, $sid, $title, $url, $description, $modifysubmitter, $topicid_card) = sql_fetch_row($resultX)) {
        ...
        $result2 = sql_query("SELECT cid, sid, title, url, description, submitter, topicid_card 
                              FROM " . $links_DB . "links_links 
                              WHERE lid='$lid'");
        ...
        $result9 = sql_query("SELECT topictext 
                              FROM " . sql_prefix('topics') . " 
                              WHERE topicid='$oritopicid_card'");

        list($oritopic) = sql_fetch_row($result9);

        $result9b = sql_query("SELECT topictext 
                               FROM " . sql_prefix('topics') . " 
        ...
        echo '<div class="card-deck-wrapper mt-3">
            <div class="card-deck">
        ...
        global $links_topic;
        if ($links_topic) {
            echo '<strong>' . translate('Sujet') . ' :</strong> ' . $oritopic . '<br />';
        }

        echo '<strong>' . translate('Catégorie :') . '</strong> ' . $origcidtitle . '<br />
                <strong>' . translate('Sous-catégorie :') . '</strong> ' . $origsidtitle . '<br />';

        if ($owneremail == '') {
            echo '<strong>' . translate('Propriétaire') . ':</strong> <span' . clformodif($origsidtitle, $sidtitle) . '>' . $owner . '</span><br/>';
        } else {
            echo '
        ...
        global $links_topic;
        if ($links_topic) {
            echo '<strong>' . translate('Sujet') . ' :</strong> <span' . clformodif($oritopic, $topic) . '>' . $topic . '</span><br />';
        }

        echo '<strong>' . translate('Catégorie :') . '</strong> <span' . clformodif($origcidtitle, $cidtitle) . '>' . $cidtitle . '</span><br />
                <strong>' . translate('Sous-catégorie :') . '</strong> <span' . clformodif($origsidtitle, $sidtitle) . '>' . $sidtitle . '</span><br />';

        if ($owneremail == '') {
            echo '<strong>' . translate('Propriétaire') . ': </strong> <span>' . $owner . '</span><br />';
        } else {
...
function LinksListBrokenLinks()
{
    ...
    if ($totalbrokenlinks == 0) {
        ...
        echo '<h3 class="mb-3">' . translate('Liens cassés rapportés par un ou plusieurs utilisateurs') . ' <span class="badge bg-danger float-end"> ' . $totalbrokenlinks . '</span></h3>
            <hr class="mb-0"/>
...
function LinksDelBrokenLinks($lid)
{
    ...
    sql_query("DELETE FROM " . $links_DB . "links_modrequest 
               WHERE lid='$lid'");
...
function LinksChangeModRequests($Xrequestid)
{
    ...
    $result = sql_query("SELECT requestid, lid, cid, sid, title, url, description, topicid_card 
                         FROM " . $links_DB . "links_modrequest 
    ...
    while (list($requestid, $lid, $cid, $sid, $title, $url, $description, $topicid_card) = sql_fetch_row($result)) {
        ...
        sql_query("UPDATE " . $links_DB . "links_links 
                   SET cid=$cid, sid=$sid, title='$title', url='$url', description='$description', topicid_card='$topicid_card' 
                   WHERE lid='$lid'");
    }
