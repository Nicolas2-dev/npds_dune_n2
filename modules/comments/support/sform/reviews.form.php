...
$m->addSelect('score', translate('Note'), $tmp, false, 5, false);

// CE CHAMPS est indispensable --- Don't remove this field
// Champ text : Longueur = 800 / TextArea / Obligatoire / Pas de Vérification
$m->addField('message', translate('Commentaire'), '', 'textarea', true, 800, 10, '', '');

// Champs nécessaires au fonctionnement du formulaire / Don't remove these fields
$m->addTitle(translate('Note'));

$m->addField('ModStart', '', $ModStart, 'hidden', false);

$m->addField('ModPath', '', $ModPath, 'hidden', false);

$m->addField('topic', '', $topic, 'hidden', false);

$m->addField('file_name', '', $file_name, 'hidden', false);

// Submit bouton and anti-spam
$m->addExtra('<tr><td align="center" colspan="2"><br />');

$m->addExtraHidden(Spam::questionSpambot() . '&nbsp;&nbsp;');

$m->addField('SubmitS', '', translate('Valider'), 'submit', false);
