...
function verif_chmod()
{
    ...
    foreach ($file_to_check as $v) {
        if (file_exists($v)) {
            ...
            $listfich .=  '<li class="list-group-item list-group-item-danger">' . ins_translate('Le fichier') . ' ' . $v . ' ' . ins_translate('est introuvable !') . '</li>';
            $stopngo = 1;
        }
...
function write_users($adminlogin, $adminpass1, $adminpass2)
{
    global $minpass, $stage7_ok;
    ...
    if ($adminlogin != '') {
        if ($adminpass1 != $adminpass2) {
            ...
            if (strlen($adminpass1) < $minpass) {
                ...
                $result1 = sql_query("UPDATE " . sql_prefix('authors') . " 
