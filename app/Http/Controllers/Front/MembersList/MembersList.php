...
class MembersList extends FrontBaseController
{
    ...
    public function index()
    {
        ...
        if ($letter != 'front') {
            echo '<table class="table table-no-bordered table-sm " data-toggle="table" data-search="true" data-show-toggle="true" data-mobile-responsive="true" data-buttons-class="outline-secondary" data-icons="icons" data-icons-prefix="fa" data-show-columns="true">
                <thead>
                    <tr>
            ...
            if ($num_rows_per_order > 0) {
                ...
                while ($temp_user = sql_fetch_assoc($result)) {
                    ...
                    if ($temp_user['femail'] != '') {
                        $useroutils .= '<a class="list-group-item text-primary text-center text-md-start" href="mailto:' . Spam::antiSpam($temp_user['femail'], 1) . '" target="_blank" title="' . translate('Email') . '" ><i class="fa fa-at fa-2x align-middle fa-fw"></i><span class="ms-3 d-none d-md-inline">' . translate('Email') . '</span></a>';
                    }
                    ...
                    if (($open_user == 1 and $user) || ($admin)) {
                        if ($open_user == 0) {
                            ...
                            echo '<tr class="table-danger" title="' . translate('Connexion non autorisée') . '" data-bs-toggle="tooltip">
                        ...
                        if ($sortby != 'user_from ASC') {
                            if ($admin) {
                                if (Forum::isBadMailUser($temp_user['uid']) === true) {
                                    ...
                                    echo '<td><small>' . Spam::pregAntiSpam($temp_user['email']) . '</small></td>';
                                }
                            } else {
            ...
            if ($user) {
                echo '<div class="mt-3 card card-block-small">
            ...
            if ($num_rows_per_order > $pagesize) {
                ...
                echo Paginator::paginateSingle('memberslist.php?letter=' . $letter . '&amp;sortby=' . $sortby . '&amp;list=' . $list . '&amp;gr_from_ws=' . $gr_from_ws . '&amp;page=', '', $nbPages, $current, $adj = 3, '', '');
            } else {
    ...
    function SortLinks($letter)
    {
        ...
        echo translate('Classé par ordre de : ') . ' '; // <-- débile bien le genre de code de npds!!!
