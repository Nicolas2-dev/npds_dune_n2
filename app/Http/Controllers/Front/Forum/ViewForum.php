...
class ViewForum extends FrontBaseController
{
    ...
    public function index()
    {
        ...
        if (($myrow['forum_type'] == 1) and (($myrow['forum_name'] != $forum_name) or ($Forum_passwd != $myrow['forum_pass']))) {
            ...
            for ($i = 0; $i < count($moderator_data); $i++) {
                $modera = Forum::getUserData($moderator_data[$i]);
            ...
            if ($forum_access != 9) {
                ...
                if ($forum_access == 2) {
                    if (!Forum::userIsModerator($userR[0], $userR[2], $forum_access)) {
                        $allow_to_post = false;
                    }
                }
            ...
            if ($myrow = sql_fetch_assoc($result)) {
                echo '<h4 class="my-2">' . translate('Sujets') . ' <span class="text-body-secondary">' . $mess_closoled . '</span></h4>
                ...
                do {
                    ...
                    $replys = Forum::getTotalPosts($forum, $myrow['topic_id'], 'topic', $Mmod);
                    $replys--;
                    ...
                    if ($replys >= 0) {
                        ...
                        if ($Sredirection) {
                            ...
                            echo '<td class="small">' . Forum::getLastPost($myrow['topic_id'], "topic", "infos", $Mmod) . '</td>
                            </tr>';
                        }
                    }
            ...
            if ($user) {
                echo '<i class="far fa-file-alt fa-lg faa-shake animated text-primary"></i> = ' . translate('Les nouvelles contributions depuis votre dernière visite.') . '<br />
                ...
                echo '<i class="fas fa-file-alt fa-lg text-primary"></i> = ' . translate('Plus de') . ' ' . $hot_threshold . ' ' . translate('Contributions') . '<br />
                <i class="far fa-file-alt fa-lg text-primary"></i> = ' . translate('Contributions') . '.<br />';
            }

            echo '<i class="fa fa-lock fa-lg text-danger"></i> = ' . translate('Ce sujet est verrouillé : il ne peut accueillir aucune nouvelle contribution.') . '<br />
            </blockquote>';
            ...
            if (($cache_obj->genereting_output == 1) or ($cache_obj->genereting_output == -1) or (!$SuperCache)) {
                echo '<form class="my-3" action="viewforum.php" method="post">
                    <div class="mb-3 row">
                        <div class="col-12">
