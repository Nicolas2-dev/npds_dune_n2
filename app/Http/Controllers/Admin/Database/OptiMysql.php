...
class OptiMysql extends AdminBaseController
{
    /**
    ...
    public function index()
    {
        ...
        if (!empty($dateopt) && !empty($houropt)) {
            $last_opti = adm_translate('Dernière optimisation effectuée le')
                . ' : ' . $dateopt
                . ' ' . adm_translate('à')
                . ' ' . $houropt . "<br />\n";
        }
        ...
        if (sql_num_rows($result)) {
            ...
            while ($row = sql_fetch_assoc($result)) {
                ...
                if ($gain == 0) {
                    $li_tab_opti .= '<tr class="table-success">
                    ...
                    $li_tab_opti .= '<tr class="table-danger">
                        <td align="right">' . $row['Name'] . '</td>
                        <td align="right">' . $total . ' Ko</td>
        ...
        echo adm_translate('A ce jour, vous avez effectué ') . ' ' . $countopt . ' optimisation(s) ' . adm_translate(' et réalisé un gain global de ') . ' ' . $gainopt . ' Ko.</p>
