...
class ModuleInstall extends AdminBaseController
{
    /**
    ...
    protected function initialize()
    {
        ...
        if ($ModInstall != '' && $ModDesinstall == '') {
            ...
            if (file_exists("modules/" . $ModDesinstall . "/install.conf.php")) {
                ...
                $display .= '<p><strong>' . adm_translate('La désinstallation automatique des modules n\'est pas prise en charge à l\'heure actuelle.') . '</strong>
    ...
    public function nmig_WriteSql($sql, $path_adm_module, $name_module, $affich, $icon)
    {
        ...
        if (isset($erreur)) {
            ...
            if ($path_adm_module != '') {
                ...
                $ck = sql_query("SELECT fnom FROM " . sql_prefix('fonctions') . " 
    ...
    public function nmig_AlertBloc($blocs, $name_module)
    {
        ...
        $display .= '<div class="text-center mb-3">
    ...
    public function nmig_WriteBloc($blocs, $posbloc, $name_module)
    {
        ...
        if ($posbloc) {
            ...
            for ($i = 0; $i < count($blocs[0]) && !isset($erreur); $i++) {
                sql_query("INSERT INTO " . $posbloc . sql_prefix('blocks') . "  (`id`, `title`, `content`, `member`, `" . $posblocM . "index`, `cache`, `actif`, `aide`) 
