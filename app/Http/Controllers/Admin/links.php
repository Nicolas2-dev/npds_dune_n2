...
function links()
{
    ...
    $result = sql_query("SELECT * FROM " . sql_prefix('links_modrequest') . " 
                         WHERE brokenlink=1");

    $totalbrokenlinks = sql_num_rows($result);

    $result2 = sql_query("SELECT * FROM " . sql_prefix('links_modrequest') . " 
                          WHERE brokenlink=0");
    ...
    if ($numrows > 0) {
        ...
        echo '<label class="col-form-label col-sm-4" for="cat">' . adm_translate('Catégorie') . '</label>
            <div class="col-sm-8">
...
function LinksModLink($lid)
{
    ...
    echo '<form action="admin.php" method="post" name="adminForm" id="linksmodlink">
        <div class="mb-3 row">
            <label class="col-form-label col-sm-4 " for="title">' . adm_translate('Titre de la page') . '</label>
            <div class="col-sm-8">
    ...
    if ($recordexist == 0) {
        echo '<h3 class="mb-3">' . adm_translate('Ajouter un Editorial') . '</h3>
    ...
    $arg1 = 'var formulid = ["linksmodlink","linkseditorial"];
        inpandfieldlen("title",100);
        inpandfieldlen("url",255);
...
function LinksListBrokenLinks()
{
    ...
    if ($totalbrokenlinks == 0) {
        ...
        echo '<table id="tad_linkbrok" data-toggle="table" data-striped="true" data-search="true" data-show-toggle="true" data-mobile-responsive="true" data-buttons-class="outline-secondary" data-icons="icons" data-icons-prefix="fa">
        <thead>
            <tr>
...
function LinksDelBrokenLinks($lid)
{
    ...
    global $aid;
    Ecr_Log('security', sprintf('DeleteBrokensLinks(%s) by AID : %s', $lid, $aid), '');
...
{
    sql_query("DELETE FROM " . sql_prefix('links_modrequest') . " 
               WHERE lid='$lid' AND brokenlink='1'");
...
function LinksChangeModRequests($Xrequestid)
{
    ...
    global $aid;
    Ecr_Log('security', sprintf('UpdateModRequestLinks(%s) by AID : %s', $Xrequestid, $aid), '');
...
function LinksModLinkS($lid, $title, $url, $xtext, $name, $email, $hits, $cat)
{
    ...
    global $aid;
    Ecr_Log('security', sprintf('UpdateLinks(%s, %s) by AID : %s', $lid, $title, $aid), '');
...
function LinksDelLink($lid)
{
    ...
    global $aid;
    Ecr_Log('security', sprintf('DeleteLinks(%s) by AID : %s', $lid, $aid), '');
...
function LinksModCat($cat)
{
    ...
    if ($cat[1] == 0) {
        ...
        echo '<form action="admin.php" method="get" id="linksmodcat">
            <div class="mb-3 row">
...
function LinksModCatS($cid, $sid, $sub, $title, $cdescription)
{
    if ($sub == 0) {
        ...
        global $aid;
        Ecr_Log('security', sprintf('UpdateCatLinks(%s, %s) by AID : %s', $cid, $title, $aid), '');
    } else {
        ...
        global $aid;
        Ecr_Log('security', sprintf('UpdateSubCatLinks(%s, %s) by AID : %s', $cid, $title, $aid), '');
    }
...
function LinksDelCat($cid, $sid, $sub, $ok = 0)
{
    if ($ok == 1) {
        if ($sub > 0) {
            ...
            global $aid;
            Ecr_Log('security', sprintf('DeleteSubCatLinks(%s) by AID : %s', $sid, $aid), '');
        } else {
            ...
            global $aid;
            Ecr_Log('security', sprintf('DeleteCatLinks(%s) by AID : ', $cid, $aid), '');
        }
...
function LinksDelNew($lid)
{
    ...
    global $aid;
    Ecr_Log('security', sprintf('DeleteNewLinks(%s) by AID : %s', $lid, $aid), '');
...
function LinksAddCat($title, $cdescription)
{
    ...
    if ($numrows > 0) {
        ...
        global $aid;
        Ecr_Log('security', sprintf('AddCatLinks(%s) by AID : %s', $title, $aid), '');
...
{
    $result = sql_query("SELECT cid FROM " . sql_prefix('links_subcategories') . " 
                         WHERE title='$title' 
                         AND cid='$cid'");
    ...
    if ($numrows > 0) {
        ...
        global $aid;
        Ecr_Log('security', sprintf('AddSubCatLinks(%s) by AID : %s', $title, $aid), '');
...
function LinksAddEditorial($linkid, $editorialtitle, $editorialtext)
{
    ...
    Ecr_Log('security', sprintf('AddEditorialLinks(%s, %s) by AID : %s', $linkid, $editorialtitle, $aid), '');
...
function LinksModEditorial($linkid, $editorialtitle, $editorialtext)
{
    ...
    global $aid;
    Ecr_Log('security', sprintf('ModEditorialLinks(%s, %s) by AID : %s', $linkid, $editorialtitle, $aid), '');
...
function LinksDelEditorial($linkid)
{
    ...
    global $aid;
    Ecr_Log('security', sprintf('DeteteEditorialLinks(%s) by AID : %s', $linkid, $aid), '');
...
function LinksAddLink($new, $lid, $title, $url, $cat, $xtext, $name, $email, $submitter)
{
    ...
    if ($numrows > 0) {
        ...
        global $aid;
        Ecr_Log('security', sprintf('AddLinks(%s) by AID : %s', $title, $aid), '');
