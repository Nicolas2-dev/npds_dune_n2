...
class Metalang extends AdminBaseController
{
    /**
    ...
    public function list_meta($meta, $type_meta)
    {
        $sel = '';

        $list = '<select class="form-select" name="meta" onchange="window.location=eval(\'this.options[this.selectedIndex].value\')">
    ...
    public function list_meta_type()
    {
        $list = '<select class="form-select" name="type_meta" onchange="window.location=eval(\'this.options[this.selectedIndex].value\')">
    ...
    public function list_type_meta($type_meta)
    {
        $sel = '';
    ...
    public function Edit_metaLang()
    {
        ...
        if ($Q['obligatoire'] != true) {
            echo '<form id="metalangedit" name="edit_meta_lang" action="admin.php" method="post">
            ...
            if ($Q['type_meta'] == 'smil') {
                ...
                echo '<textarea class="form-control" id="desc" name="desc" rows="7" >' . $Q['description'] . '</textarea>
                    </div>
                </div>';
            }
    ...
    public function Creat_metaLang()
    {
        ...
        if ($type_meta) {
            ...
            if ($type_meta != "them") {
                ...
                if ($type_meta == "smil") {
                    echo '<input class="form-control" type="text" name="content" id="content" maxlength="255" required="required" />
                ...
                if ($type_meta == "meta") {
                    echo "public function MM_XYZ (\$arg) {\n   global \sql_prefix('');\n   \$arg = Metalang::argFilter(\$arg);\n\n   return(\$content);\n}";
                }
        ...
        $arg1 = 'var formulid = ["metalangcreat"];
    ...
    public function kill_metaLang($nbr, $action)
    {
        $i = 0;
    ...
    public function Maj_Bdd_ML($Maj_Bdd_ML, $def, $content, $type_meta, $type_uri, $uri, $desc)
    {
        if ($type_uri == 'plus') {
            $type_uri = '+';
        } else {
            ...
            if ($Q['def']) {
                ...
                if ($def != '') {
                    sql_query("INSERT INTO " . sql_prefix('metalang') . " 
                            SET def='" . $def . "', content='$content', type_meta='" . $type_meta . "', type_uri='" . $type_uri . "', uri='" . $uri . "', description='" . $desc . "', obligatoire='0'");
                }
