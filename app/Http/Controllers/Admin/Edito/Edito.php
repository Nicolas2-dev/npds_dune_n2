...
class Edito extends AdminBaseController
{
    /**
        ...
        switch ($op) {
            ...
            case 'Edito_load':
                if ($edito_type == 'G') {
                    ...
                    $path = 'storage/static/edito.txt';

                    if (file_exists($path)) {
                        $fp = fopen($path, 'r');

                        if (filesize($path) > 0) {
                            $Xcontents = fread($fp, filesize($path));
                        }
                        fclose($fp);
                    }
                } elseif ($edito_type == 'M') {
    ...
    public function edito($edito_type, $contents, $Xaff_jours, $Xaff_jour, $Xaff_nuit)
    {
        ...
        if ($contents == '') {
            echo '<form id="fad_edi_choix" action="admin.php?op=Edito_load" method="post">
                <fieldset>
            ...
            echo '<form id="admineditomod" action="admin.php" method="post" name="adminForm">
                <fieldset>
            ...
            $arg1 = 'var formulid = ["admineditomod"];';
