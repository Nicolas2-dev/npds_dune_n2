...
$m->addField('name', translate('Votre véritable identité'), $name, 'text', false, 60, '', '');

$m->addExtender('name', '', '<span class="help-block"><span class="float-end" id="countcar_name"></span></span>');

$m->addField('email', translate('Véritable adresse Email'), $email, 'email', true, 60, '', '');
...
$m->addCheckbox('user_viewemail', translate('Autoriser les autres utilisateurs à voir mon Email'), "1", false, false);
...
if ($smilies) {
    ...
    $m->addExtender('user_avatar', 'onkeyup="showimage();" onchange="showimage();"', '<img class="img-thumbnail n-ava mt-3" src="' . $direktori . '/blank.gif" name="avatar" alt="avatar" />');

    $m->addField('B1', 'B1', '', 'hidden', false);
}

$m->addField('user_from', translate('Votre situation géographique'), StripSlashes($user_from ?? ''), 'text', false, 100, '', '');

$m->addExtender('user_from', '', '<span class="help-block"><span class="float-end" id="countcar_user_from"></span></span>');

$m->addField('user_occ', translate('Votre activité'), StripSlashes($user_occ ?? ''), 'text', false, 100, '', '');

$m->addExtender('user_occ', '', '<span class="help-block"><span class="float-end" id="countcar_user_occ"></span></span>');

$m->addField('user_intrest', translate('Vos centres d\'intérêt'), StripSlashes($user_intrest ?? ''), 'text', false, 150, '', '');

$m->addExtender('user_intrest', '', '<span class="help-block"><span class="float-end" id="countcar_user_intrest"></span></span>');

$m->addField('user_sig', translate('Signature'), StripSlashes($user_sig ?? ''), 'textarea', false, 255, '7', '');

$m->addExtender('user_sig', '', '<span class="help-block">' . translate('(255 characters max. Type your signature with HTML coding)') . '<span class="float-end" id="countcar_user_sig"></span></span>');

// --- MEMBER-PASS
if ($memberpass) {
    $m->addField('pass', translate('Mot de passe'), '', 'password', true, 40, '', '');
    ...
    $m->addField('vpass', translate('Entrez à nouveau votre mot de Passe'), '', 'password', true, 40, '', '');
}

// --- MEMBER-PASS
$m->addCheckbox('user_lnl', translate('S\'inscrire à la liste de diffusion du site'), "1", false, true);
...
// Champ Hidden
$m->addField('op', '', 'new user', 'hidden', false);

// --- CHARTE du SITE
$m->addCheckbox('charte', '<a href="static.php?op=charte.html" target="_blank">' . translate('Vous devez accepter la charte d\'utilisation du site') . '</a>', "1", true, false);
...
$arg1 = 'var formulid = ["register"];
    ' . Js::autoComplete('aruser', 'uname', 'users', '', '0');
