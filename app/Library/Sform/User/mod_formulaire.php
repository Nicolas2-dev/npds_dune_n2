...
$m->addMess(translate('* Désigne un champ obligatoire'));

//$m->addFormFieldSize(50);

$m->addField('name', translate('Votre véritable identité') . ' ' . translate('(optionnel)'), $userinfo['name'], 'text', false, 60, '', '');

$m->addExtender('name', '', '<span class="help-block"><span class="float-end" id="countcar_name"></span></span>');

$m->addField('email', translate('Véritable adresse Email'), $userinfo['email'], 'email', true, 60, '', '');

$m->addExtender('email', '', '<span class="help-block">' . translate('(Cette adresse Email ne sera pas divulguée, mais elle nous servira à vous envoyer votre Mot de Passe si vous le perdez)') . '<span class="float-end" id="countcar_email"></span></span>');

$m->addField('femail', translate('Votre adresse mèl \'truquée\''), $userinfo['femail'], 'email', false, 60, "", "");

$m->addExtender('femail', '', '<span class="help-block">' . translate('(Cette adresse Email sera publique. Vous pouvez saisir ce que vous voulez mais attention au Spam)') . '<span class="float-end" id="countcar_femail"></span></span>');
...
$m->addCheckbox('user_viewemail', translate('Autoriser les autres utilisateurs à voir mon Email'), 1, false, $checked);

$m->addField('url', translate('Votre page Web'), $userinfo['url'], 'url', false, 100, '', '');

$m->addExtender('url', '', '<span class="help-block"><span class="float-end" id="countcar_url"></span></span>');
...
if ($subscribe) {
    //proto
    if (Forum::isBadMailUser($userinfo['uid']) === false) {
        ...
        $m->addCheckbox('usend_email', translate('M\'envoyer un Email lorsqu\'un message interne arrive'), 1, false, $checked);
    }
}
...
if ($member_invisible) {
    ...
    $m->addCheckbox('uis_visible', translate('Membre invisible') . " (" . translate('pas affiché dans l\'annuaire, message à un membre, ...') . ")", 1, false, $checked);
}
...
if (Forum::isBadMailUser($userinfo['uid']) === false) {
    ...
    $m->addCheckbox('user_lnl', translate('S\'inscrire à la liste de diffusion du site'), 1, false, $checked);
}

// ---- AVATAR
if ($smilies) {
    if (stristr($userinfo['user_avatar'], 'users_private')) {
        $m->addField('user_avatar', translate('Votre Avatar'), $userinfo['user_avatar'], 'show-hidden', false, 30, '', '');

        $m->addExtender('user_avatar', '', '<img class="img-thumbnail n-ava" src="' . $userinfo['user_avatar'] . '" name="avatar" alt="avatar" /><span class="ava-meca lead"><i class="fa fa-angle-right fa-lg text-body-secondary mx-3"></i></span><img class="ava-meca img-thumbnail n-ava" id="ava_perso" src="#" alt="Your next avatar" />');
    } else {
        global $theme;
    ...
    $m->addUpload('B1', '', '30', $taille_fichier);

    $m->addExtender('B1', '', '<span class="help-block text-end">Taille maximum du fichier image :&nbsp;=>&nbsp;<strong>' . $taille_fichier . '</strong> octets et <strong>' . $avatar_size . '</strong> pixels</span>');

    $m->addExtra('<div id="avatarPreview" class="preview"></div>');

    $m->addCheckbox('raz_avatar', translate('Revenir aux avatars standards'), 1, false, false);
}

$m->addField('user_from', translate('Votre situation géographique'), $userinfo['user_from'], 'text', false, 100, '', '');

$m->addExtender('user_from', '', '<span class="help-block text-end" id="countcar_user_from"></span>');

$m->addField('user_occ', translate('Votre activité'), $userinfo['user_occ'], 'text', false, 100, '', '');

$m->addExtender('user_occ', '', '<span class="help-block text-end" id="countcar_user_occ"></span>');

$m->addField('user_intrest', translate('Vos centres d\'intérêt'), $userinfo['user_intrest'], 'text', false, 150, '', '');

$m->addExtender('user_intrest', '', '<span class="help-block text-end" id="countcar_user_intrest"></span>');
...
$m->addCheckbox('attach', translate('Afficher la signature'), 1, false, $checked);

$m->addField('user_sig', translate('Signature'), $userinfo['user_sig'], 'textarea', false, 255, 4, '', '');

$m->addExtender('user_sig', '', '<span class="help-block">' . translate('(255 caractères max. Entrez votre signature (mise en forme html))') . '<span class="float-end" id="countcar_user_sig"></span></span>');
// ---- SIGNATURE

$m->addField('bio', translate('Informations supplémentaires'), $userinfo['bio'], 'textarea', false, 255, 4, '', '');

$m->addExtender('bio', '', '<span class="help-block">' . translate('(255 caractères max). Précisez qui vous êtes, ou votre identification sur ce site)') . '<span class="float-end" id="countcar_bio"></span></span>');

$m->addField('pass', translate('Mot de passe'), '', 'password', false, 40, '', '');
...
$m->addExtender('pass', '', '<span class="help-block text-end" id="countcar_pass"></span>');

$m->addField('vpass', translate('Entrez à nouveau votre mot de Passe'), '', 'password', false, 40, '', '');

$m->addExtender('vpass', '', '<span class="help-block text-end" id="countcar_vpass"></span>');
